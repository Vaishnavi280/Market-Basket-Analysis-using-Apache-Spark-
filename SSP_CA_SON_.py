...
def CountOfCandidate(baskets, candidates):
	itemCountDict = {}
...
def generateFrequentItems(baskets, candidates, threshold):
	kItemCountDict = {}
	for candidate in candidates:
		candidate = set(candidate)
		key = tuple(sorted(candidate))
...
def generateCandidateItems(frequentItems, k):
	combinations1 = list()
...
def apriori(baskets, support, totalCount):
	...
	while len(frequentItems) != 0:
		if k==2:
			candidateFrequentItems = generateCandidatePairs(frequentItems)
		else:
			candidateFrequentItems = generateCandidateItems(frequentItems, k)

		newFrequentItems = generateFrequentItems(baskets, candidateFrequentItems, threshold)
...
#case=1
#inputFile="/Users/vaish/Desktop/Elective/ssp/CA2/Data/Small1.csv"
#support=4

fileNameWithExt = os.path.basename(inputFile)
fileName = os.path.splitext(fileNameWithExt)[0]
outputFile = "SSP_CA_SON_" + fileName + ".case" + str(case) + "-" + str(support) + ".txt"
...
#Phase 2
mapOutput2 = allBaskets.mapPartitions(lambda baskets : CountOfCandidate(baskets, reduceOutput1))
